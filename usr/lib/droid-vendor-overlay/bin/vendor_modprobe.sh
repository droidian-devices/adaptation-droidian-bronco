...
# If kernel version contains "bronco", all modules are built-in
if uname -r | grep -q "bronco"; then
    echo "Info: Bronco kernel detected, Skipping vendor modules loading"
    setprop vendor.all.modules.ready 1
    exit 0
fi

echo "Info: Start loading vendor modules"
